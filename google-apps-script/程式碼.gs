...
function ensureSheets_() {
  ...
  ensureInquiryTextFormat_(ss);
  seedInquiryRecipients_();
}



// V27：客戶表單中，除提交時間外，所有可能含數字的使用者輸入欄位固定為純文字。
// 這可保留開頭 0、結尾 0、長數字、LINE ID、電話、備註中的數字格式。
function ensureInquiryTextFormat_(ss) {
  const sh = ss.getSheetByName(TABS.inquiries);
  if (!sh) return;

  const rows = Math.max(sh.getMaxRows() - 1, 1);
  sh.getRange(2,1,rows,1).setNumberFormat('@');   // 編號
  sh.getRange(2,3,rows,10).setNumberFormat('@'); // C:L：表單內容＋狀態
}
...
function saveInquiry_(data) {
  ...
  // V27：所有使用者輸入內容一律以「純文字」寫入。
  // 避免 Google Sheets 將 0900、00123、1.20、0000 等自動轉成數字而吃掉 0。
  const textValues = [
  ...
  // A 編號：文字；B 提交時間：日期時間；C:J 表單內容：純文字；K:L 狀態：文字
  sh.getRange(row,1).setNumberFormat('@').setValue(String(id));
  sh.getRange(row,2).setValue(now);
  sh.getRange(row,3,1,8).setNumberFormat('@').setValues([textValues]);
  sh.getRange(row,11,1,2).setNumberFormat('@').setValues([['未讀','未處理']]);
...
function readInquiries_() {
  ...
  const range = sh.getRange(2,1,sh.getLastRow()-1,12);
  ...
  return rows.map((r,i) => {
    ...
    return {
      ...
      storeName: String(d[2] || ''),
      contactName: String(d[3] || ''),
      phone: String(d[4] || ''),
      line: String(d[5] || ''),
      email: String(d[6] || ''),
      businessStatus: String(d[7] || ''),
      service: String(d[8] || ''),
      note: String(d[9] || ''),
      readStatus: String(d[10] || '未讀'),
      processStatus: String(d[11] || '未處理')
    };
