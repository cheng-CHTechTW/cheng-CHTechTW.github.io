...
function doPost(e) {
  try {
    ...
    if (action === 'importCurrentInvoiceFaqs') {
      return json_({ok:true, message:importCurrentInvoiceFaqs()});
    }
    return json_({ok:false, message:'UNKNOWN_ACTION'});
  } catch (err) {
    return json_({ok:false, message:String(err.message || err)});
...
function setupInvoiceQaSheet() {
  ...
  return '已建立電子發票 QA 工作表，共 ' + demo.length + ' 題';
}

function importCurrentInvoiceFaqs() {
  const sh = getSheet_();
  const current = listFaqs_(true);
  const existing = new Set(current.map(function(x) {
    return String(x.question || '').trim();
  }));
...
function listFaqs_(includeDisabled) {
  ...
  return rows.map(function(r) {
    return {
      ...
      question:String(r[2] || ''),
      answer:String(r[3] || ''),
      sort:Number(r[4] || 9999),
      enabled:String(r[5] || 'Y'),
      updatedAt:r[6] instanceof Date ? r[6].toISOString() : String(r[6] || '')
    };
  }).filter(function(x) {
    return includeDisabled || x.enabled.toUpperCase() !== 'N';
  }).sort(function(a,b) {
...
function saveFaq_(d) {
  ...
  if (last >= 2) {
    const ids = sh.getRange(2,1,last-1,1).getValues().map(function(r) {
      return String(r[0] || '');
    });
